...
class DataFetcher:
    def __init__(self, js_file_path='第十二题加密.js'):
        """初始化DataFetcher，加载JS文件路径"""
        self.js_file_path = os.path.abspath(js_file_path) if js_file_path else os.path.abspath('第十二题加密.js')
    ...
    def get_token(self):
        ...
        // 修复JS文件中的错误 - 确保在访问属性前对象已定义
        // 创建一个安全的执行环境
        try {{
            eval(jsContent);
            
            // 调用函数获取结果
            const result = global.s ? s() : (typeof s !== 'undefined' ? s() : []);
            if (result && result.length >= 2) {{
                ...
                // 如果没有找到s函数，尝试其他可能的函数名
                const possibleFunctions = ['getToken', 'generateToken', 't', 'getTokenData'];
                let found = false;
                for (const funcName of possibleFunctions) {{
                    if (typeof global[funcName] === 'function') {{
                        try {{
                            const res = global[funcName]();
                            if (res && res.length >= 2) {{
                                console.log(JSON.stringify({{
                                    'token': res[0],
                                    'timestamp': res[1]
        ...
        try:
            ...
            if result.returncode == 0 and result.stdout.strip():
                ...
                # 如果执行失败，返回默认值
                return ['default_token', str(int(time.time() * 1000))]
    ...
    def decrypt_data(self, encrypted_data):
        ...
        try {{
            ...
            // 尝试多种可能的解密函数名
            const possibleFunctions = ['updatePageContent', 'decrypt', 'decode', 'parseData', 'decryptData'];
            let found = false;
            
            for (const funcName of possibleFunctions) {{
                if (typeof global[funcName] === 'function') {{
                    try {{
                        const result = global[funcName]('{encrypted_data}');
                        if (result) {{
            ...
            if (!found) {{
                // 如果找不到解密函数，返回原始数据
                console.log(JSON.stringify({{ 'data': '{encrypted_data}' }}));
            }}
        }} catch (e) {{
            // 出现错误时返回原始数据
            console.log(JSON.stringify({{ 'data': '{encrypted_data}', 'error': e.message }}));
        }}
        """
...
def fetch_page_data(page, token):
    """获取单页数据"""
    url = f"https://www.mashangpa.com/api/problem-detail/12/data/?page={page}"
    headers = {
        "accept": "*/*",

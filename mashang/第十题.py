...
def get_url1(k):
  with open('第十题加密.js', 'r', encoding='utf-8') as f:
    j = f.read()
    ctx = execjs.compile(j)
    result = ctx.call('get_problem_detail', k)
  return result

def get_url(page):
    # 生成t参数
    base_url = "https://www.mashangpa.com/api/problem-detail/10/data/?page="+str(page)
    t_parameter = get_url1(page)  # 您需要根据实际逻辑调整
    url_with_params = f"{base_url}&t={t_parameter}"
...
def submit_answer(answer):
    ...
    提交答案到服务器
    """
    url = "https://www.mashangpa.com/problem/10/submit/"
    headers = {
        "accept": "*/*",
        "accept-language": "zh-CN,zh;q=0.9,en;q=0.8,en-GB;q=0.7,en-US;q=0.6",
        "cache-control": "no-cache",
